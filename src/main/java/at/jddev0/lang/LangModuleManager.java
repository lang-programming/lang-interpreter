...
final class LangModuleManager {
	...
	private DataObject loadUnload(boolean load, String moduleFileOrName, List<DataObject> args, final int CALLER_SCOPE_ID) {
		...
		try {
			...
			String[] langArgs = LangUtils.combineArgumentsWithoutArgumentSeparators(args, interpreter, -1, SCOPE_ID).stream().map(ele ->
					interpreter.conversions.toText(ele, -1, CALLER_SCOPE_ID)).
